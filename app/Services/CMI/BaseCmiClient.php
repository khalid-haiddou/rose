...
use Exception;
use App\Services\CMI\Exception\InvalidArgumentException;
...
class BaseCmiClient implements CmiClientInterface
{
     ...
     */
    public function __construct($requireOpts = [], $baseUrl = self::DEFAULT_API_BASE)
    {
     ...
     * @return string hash
     */
    public function generateHash1($storeKey = null): string
    {
        if ($storeKey === null) {
            $storeKey = $this->requireOpts['storekey'] ?? '';
        }

        // CMI's expected hash input order
        $orderedKeys = [
            'clientid', 'oid', 'amount', 'okUrl', 'failUrl', 'rnd',
            'hashAlgorithm', 'storetype', 'lang', 'currency', 'email',
            'BillToName', 'BillToCompany', 'BillToStreet12', 'BillToCity',
            'BillToStateProv', 'BillToPostalCode', 'BillToCountry', 'tel',
            'CallbackURL'
        ];
        ...
        foreach ($orderedKeys as $key) {
            $value = $this->requireOpts[$key] ?? '';
            $escapedValue = str_replace('|', '\\|', str_replace('\\', '\\\\', trim($value)));
